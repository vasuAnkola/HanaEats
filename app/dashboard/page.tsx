...
export default async function DashboardPage() {
    ...
    return (
      <div>
        ...
        <div className="p-6 space-y-6">
          {/* Hero banner */}
          <div className="relative overflow-hidden rounded-2xl bg-gradient-to-br from-[#5C432B] to-[#D98C3B] px-8 py-7 text-white shadow-lg">
            ...
            <p className="text-sm font-medium text-brand-gold mb-1">Welcome back,</p>
            <h1 className="text-2xl font-bold">{name} 👋</h1>
            <p className="text-brand-gold text-sm mt-1">Here's your platform overview for today.</p>
      ...
      <div>
        ...
        <div className="p-6 space-y-6">
          ...
          <div className="relative overflow-hidden rounded-2xl bg-gradient-to-br from-[#5C432B] to-[#D98C3B] px-8 py-7 text-white shadow-lg">
            ...
            <p className="text-sm font-medium text-brand-gold mb-1">Welcome back,</p>
            <h1 className="text-2xl font-bold">{name} 👋</h1>
            <p className="text-brand-gold text-sm mt-1">Manage your outlets, team, and operations.</p>
            <div className="absolute right-8 top-1/2 -translate-y-1/2 opacity-10">
  ...
  return (
    <div>
      <Header />
      <div className="p-6">
        <div className="rounded-2xl bg-gradient-to-br from-[#5C432B] to-[#D98C3B] px-8 py-7 text-white">
          <h1 className="text-xl font-bold">Welcome to HANAEats 👋</h1>
          <p className="text-brand-gold text-sm mt-1">You're logged in as {role}.</p>
        </div>
      </div>
    </div>
  );
...
function StatCard({
  ...
  const styles = {
    blue:    { bg: "bg-brand-primary",    light: "bg-brand-section border-brand-section",    text: "text-brand-primary" },
...
function QuickActions({ role }: { role: UserRole }) {
  const actions =
    role === "super_admin"
      ? [
          { label: "Add Tenant", href: "/dashboard/tenants/new", desc: "Register a new business", icon: Building2, color: "text-brand-orange bg-brand-section" },
      ...
      : [
          { label: "Add Outlet", href: "/dashboard/outlets/new", desc: "Open a new branch", icon: MapPin, color: "text-brand-orange bg-brand-section" },
      ...
      <div className="p-3 space-y-1">
        {actions.map((a) => (
          <a
            key={a.href}
            href={a.href}
            className="flex items-center gap-3 p-3 rounded-xl hover:bg-brand-section transition-colors group"
          >
            <div className={`w-9 h-9 rounded-lg flex items-center justify-center flex-shrink-0 ${a.color}`}>
              <a.icon className="w-4 h-4" />
            </div>
            <div className="flex-1 min-w-0">
              <p className="text-sm font-medium text-gray-800 group-hover:text-brand-primary">{a.label}</p>
              <p className="text-xs text-gray-400">{a.desc}</p>
            </div>
            <ArrowRight className="w-4 h-4 text-gray-300 group-hover:text-brand-light transition-colors flex-shrink-0" />
          </a>
        ))}
      </div>
    </div>
...
function PlatformHealth({ stats }: { stats: { tenants: number; outlets: number; users: number; countries: number } }) {
  const items = [
    { label: "Avg outlets per tenant", value: stats.tenants > 0 ? (stats.outlets / stats.tenants).toFixed(1) : "—", color: "bg-brand-orange" },
      ...
      <div className="p-5 space-y-4">
        ...
        <div className="pt-3 border-t border-gray-100">
          <div className="flex items-center justify-between mb-1.5">
            ...
            <span className="text-xs font-semibold text-brand-primary">{stats.tenants > 0 ? Math.min(100, Math.round((stats.outlets / (stats.tenants * 5)) * 100)) : 0}%</span>
          </div>
          <div className="w-full h-2 bg-gray-100 rounded-full">
            <div
              className="h-2 bg-gradient-to-r from-brand-orange to-brand-light rounded-full transition-all"
