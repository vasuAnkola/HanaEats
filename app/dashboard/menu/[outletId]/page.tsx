...
export default function OutletMenuPage({ params }: Props) {
  ...
  return (
    <div>
      ...
      <div className="p-6">
        <Link href="/dashboard/menu" className="inline-flex items-center gap-1.5 text-sm text-gray-500 hover:text-brand-primary mb-5 transition-colors">
        ...
        <div className="flex gap-5 h-[calc(100vh-220px)]">
          ...
          <div className="w-60 flex-shrink-0 flex flex-col bg-white border border-gray-100 rounded-2xl shadow-sm overflow-hidden">
            ...
            {loadingCats ? (
              <div className="flex justify-center py-10"><Loader2 className="w-5 h-5 animate-spin text-brand-gold" /></div>
            ) : categories.length === 0 ? (
              <div className="flex flex-col items-center justify-center flex-1 py-10 px-4 text-center">
                <div className="w-10 h-10 rounded-xl bg-brand-section flex items-center justify-center mb-3">
                  ...
                  <div
                    ...
                    className={`group flex items-center gap-2.5 px-3 py-2.5 rounded-xl cursor-pointer transition-all ${
                      selectedCat?.id === c.id
                        ...
                        : "hover:bg-brand-section text-gray-700"
                    }`}
                  >
                    ...
                    {selectedCat?.id === c.id ? (
                      <ChevronRight className="w-3.5 h-3.5 text-brand-gold flex-shrink-0" />
                    ) : (
                      <div className="flex items-center gap-0.5 opacity-0 group-hover:opacity-100 transition-opacity">
                        <button onClick={(e) => { e.stopPropagation(); openEditCat(c); }} className="w-6 h-6 rounded-lg hover:bg-brand-section flex items-center justify-center">
            ...
            {!selectedCat ? (
              <div className="flex-1 flex items-center justify-center bg-white border border-gray-100 rounded-2xl shadow-sm">
                <div className="text-center">
                  <div className="w-16 h-16 rounded-2xl bg-brand-section flex items-center justify-center mx-auto mb-4">
              ...
              <div className="flex flex-col h-full bg-white border border-gray-100 rounded-2xl shadow-sm overflow-hidden">
                ...
                <div className="flex-1 overflow-y-auto p-4">
                  {loadingItems ? (
                    <div className="flex justify-center py-12"><Loader2 className="w-5 h-5 animate-spin text-brand-gold" /></div>
                  ) : items.length === 0 ? (
                    <div className="flex flex-col items-center justify-center h-full text-center py-12">
                      <div className="w-14 h-14 rounded-2xl bg-brand-section flex items-center justify-center mb-4">
                    ...
                    <div className="space-y-2">
                      {items.map((item) => (
                        <div key={item.id} className={`flex items-center gap-4 p-4 rounded-xl border transition-all ${
                          item.is_available ? "border-gray-100 hover:border-brand-gold hover:shadow-sm" : "border-gray-100 bg-gray-50/60 opacity-60"
                        }`}>
                          {/* Icon */}
                          <div className={`w-11 h-11 rounded-xl flex items-center justify-center flex-shrink-0 ${item.is_available ? "bg-brand-primary" : "bg-gray-200"}`}>
                          ...
                          <div className="flex-1 min-w-0">
                            ...
                            <div className="flex items-center gap-3 mt-1 flex-wrap">
                              <span className="text-sm font-bold text-brand-primary">{parseFloat(String(item.price)).toFixed(2)}</span>
                              ...
                              {item.variant_count > 0 && <span className="text-xs text-brand-orange font-medium">{item.variant_count} variant{item.variant_count !== 1 ? "s" : ""}</span>}
                              {item.addon_group_count > 0 && <span className="text-xs text-brand-orange font-medium">{item.addon_group_count} add-on{item.addon_group_count !== 1 ? "s" : ""}</span>}
                            </div>
                          </div>
                          ...
                          <div className="flex items-center gap-2 flex-shrink-0">
                            ...
                            <Link href={`/dashboard/menu/${outletId}/items/${item.id}`}>
                              <Button variant="ghost" size="sm" className="h-8 w-8 p-0 text-gray-400 hover:text-brand-primary hover:bg-brand-section">
